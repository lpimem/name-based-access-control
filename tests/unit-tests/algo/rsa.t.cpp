...
const std::string privateKey = {"MIICdwIBADANBgkqhkiG9w0BAQEFAASCAmEwggJdAgEAAoGBAMLY2w1PmsuZNvZ4"
...
const std::string publicKey = {"MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDC2NsNT5rLmTb2eKybNaREi68R"
...
const uint8_t ciphertext[] = {0x33, 0xfb, 0x32, 0xd4, 0x2d, 0x45, 0x75, 0x3f, 0x34, 0xde, 0x3b,
...
BOOST_AUTO_TEST_CASE(EncryptionDecryption)
{
  ...
  StringSource privPipe(privateKey, true, new Base64Decoder(new FileSink(privateKeyBuffer)));
  StringSource publPipe(publicKey, true, new Base64Decoder(new FileSink(publicKeyBuffer)));
  ...
  const Buffer& encryptBuf =
    Rsa::encrypt(encodedPublicKey.buf(), encodedPublicKey.size(), plaintext, sizeof(plaintext), eparams);

  const Buffer& recvBuf = Rsa::decrypt(encodedPrivateKey.buf(),
                                       encodedPrivateKey.size(),
                                       encryptBuf.buf(),
                                       encryptBuf.size(),
                                       eparams);
  ...
  const Buffer& convBuf = Rsa::decrypt(derivedPrivateKey.buf(),
                                       derivedPrivateKey.size(),
                                       ciphertext,
                                       sizeof(ciphertext),
                                       eparams);
