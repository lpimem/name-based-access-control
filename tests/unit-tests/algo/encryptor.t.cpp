...
public:
    ...
    , encryptParams(tlv::AlgorithmAesEcb)
  {
    ...
    const uint8_t encrypted_content[] = {0x15, 0x31, 0x82, 0x2f, 0x1c, 0x08, 0x07, 0x06, 0x08,
                                         0x04, 0x74, 0x65, 0x73, 0x74, 0x83, 0x01, 0x00, 0x84,
                                         0x20, 0x13, 0x80, 0x1a, 0xc0, 0x4c, 0x75, 0xa7, 0x7f,
                                         0x43, 0x5e, 0xd7, 0xa6, 0x3f, 0xd3, 0x68, 0x94, 0xe2,
                                         0xcf, 0x54, 0xb1, 0xc2, 0xce, 0xad, 0x9b, 0x56, 0x6e,
                                         0x1c, 0xe6, 0x55, 0x1d, 0x79, 0x04};
...
public:
    ...
    , encryptParams(tlv::AlgorithmAesCbc)
  {
    ...
    const uint8_t encrypted_content[] = {0x15, 0x43, // Content
                                         0x82, 0x41, // EncryptedContent
                                         0x1c, 0x08, // KeyLocator /test
                                         0x07, 0x06, 0x08, 0x04, 0x74, 0x65, 0x73, 0x74,
                                         0x83, 0x01, // EncryptedAlgorithm
                                         0x01,       // AlgorithmAesCbc
                                         0x85, 0x10, 0x73, 0x6f, 0x6d, 0x65, 0x72, 0x61,
                                         0x6e, 0x64, 0x6f, 0x6d, 0x76, 0x65, 0x63, 0x74,
                                         0x6f, 0x72, 0x84, 0x20, // EncryptedPayLoad
...
typedef boost::mpl::list<TestDataAesCbc, TestDataAesEcb> EncryptorAesTestInputs;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ContentSymmetricEncrypt, T, EncryptorAesTestInputs)
{
  ...
  const Buffer& decryptedOutput = Aes::decrypt(input.key.buf(),
                                               input.key.size(),
                                               content.getPayload().buf(),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ContentAsymmetricEncryptSmall, T, EncryptorRsaTestInputs)
{
  ...
  encryptData(data, raw_content, sizeof(raw_content), keyName, eKey.buf(), eKey.size(), encryptParams);
  ...
  Buffer decrypted =
    Rsa::decrypt(dKey.buf(), dKey.size(), recovered.buf(), recovered.size(), encryptParams);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ContentAsymmetricEncryptLarge, T, EncryptorRsaTestInputs)
{
  ...
  EncryptParams encryptParams(type.type);
  encryptData(data, large_content, sizeof(large_content), keyName, eKey.buf(), eKey.size(), encryptParams);
  ...
  Buffer nonce =
    Rsa::decrypt(dKey.buf(), dKey.size(), bufferNonce.buf(), bufferNonce.size(), encryptParams);

  encryptParams.setAlgorithmType(tlv::AlgorithmAesCbc);
  encryptParams.setIV(encryptedPayload.getInitialVector().buf(),
                      encryptedPayload.getInitialVector().size());
  const Buffer& bufferPayload = encryptedPayload.getPayload();
  Buffer largePayload =
    Aes::decrypt(nonce.buf(), nonce.size(), bufferPayload.buf(), bufferPayload.size(), encryptParams);
